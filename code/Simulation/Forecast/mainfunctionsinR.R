...
SparsityPlotLarge <- function(B,p,k,title=NULL)
{
  text <- c()
  for(i in 1:p)
  {
    text1 <- as.expression(bquote(Phi^(.(i))))
    # text1 <- expression(Phi^(.(i)))
    text <- append(text,text1)
  ...
  L2 <- levelplot(f(abs(B)),col.regions=rgb.palette,colorkey=NULL,xlab=NULL,
                  ylab=NULL,main=list(label=title,cex=1),
                  panel=function(...){ panel.levelplot(...);panel.abline(a=NULL,b=1,v=seq(k+.5,p*k+.5,by=k),lwd=1.2)},scales=list(x=list(alternating=1,labels=text,cex=1,at=at,tck=c(0,0)),y=list(alternating=0,tck=c(0,0))))
