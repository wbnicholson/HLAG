...
MSFEs_CCM_mediumlarge_h1 = zeros(76, k);

for isim=1:76
    
disp(sprintf('now running isim= %d', isim));
        
Yraw = Ymediumlarge_trainh1(((isim-1)*rolsize+1):(rolsize*isim), 1:end);

if h>1
    for ih=1:(h-1)
        Yraw = [Yraw(1:end, 1:end) ; zeros(1, k)]; 
    end
    Yraw = [Yraw(1:end, 1:end) ; Ymediumlarge_testh1(isim,1:end)]; 
end
...
MSFEs_CCM_mediumlarge_h1(isim,1:end) = MSFE;
